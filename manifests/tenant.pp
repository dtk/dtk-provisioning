...
#    $activemq_user_final = $activemq_user
  ...
  $server_gemfile_dir = "${app_homedir}/server/current"
  $gitolite_admin_directory = "${app_homedir}/gitolite-admin/"

  if $seed_salts == true {
    dtk_server::tenant::salts{ "${config_base}/${app_user}/":
      require => File["${config_base}/${app_user}"]
    }
    $cookie_salt = dtksalt("${config_base}/${app_user}/.cookie_salt")
    $password_salt = dtksalt("${config_base}/${app_user}/.password_salt")
  }
...
define dtk_server::tenant::salts(
  $path=$name
)
{
  $cookie_salt = dtksalt("${path}/.cookie_salt")
  $password_salt = dtksalt("${path}/.password_salt")
}
